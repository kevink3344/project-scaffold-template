...
const stmtCheckExists = db.prepare('SELECT 1 FROM users WHERE sub = $sub')
...
function upsertAndGetUser(user) {
  const now = new Date().toISOString()
  const isNew = !stmtCheckExists.get({ $sub: user.sub })
  stmtUpsertUser.run({
    $sub:         user.sub,
    $given_name:  user.given_name  || '',
    $family_name: user.family_name || '',
  ...
  const result = stmtGetUser.get({ $sub: user.sub })
  if (isNew) fireNewUserWebhook(result)
  return result
}
